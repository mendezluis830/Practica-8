...
listen *create_listen(){
    listen *l = (listen*)malloc(sizeof(listen));
    l->head = l->tail->next;
    l->tail = l->head->prev;
    l->num = 0;
    return l;
...
dnode* search_dnode_listen(listen *l, int pos){
...
bool remove_init(listen *l){
    if(l == NULL || is_empty_listen(l)) return false;
    if(l->head ==l->tail){
        remove_dnode(l->head);
        l->head = l->tail = NULL;
        l->num = 0;
